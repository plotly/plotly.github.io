...
In the second plot (Scenario 2), the averages across the campaigns are very similar to one another and all very close to the overall average. This data was actually simulated such that there was no difference in the underlying group averages, but as you can see on the plot, there are differences in our estimates of these averages using the sampled data. We use ANOVA to try and test for whether our data arose from a situation like Scenario 1 (in which there is a true difference in the underlying means) or Scenario 2 (in which there is no difference in the underlying means). 

Sometimes, you might be convinced by graphing the data alone that there is a difference among the group means, such as with the data shown for Scenario 1. However, you can imagine an additional scenario in which the visualization isn’t so clear; there might be a difference between the means, but based on visualization alone it is difficult to gauge whether we think there is a difference or not. Thus, tests such as ANOVA provide objective tools to quantify the size of the difference.


### Calculating the one-way ANOVA test statistic

The one-way ANOVA test uses information about how far each group average is away from the overall average to quantify differences across the groups. If the group-level averages are far from the overall average (Scenario 1), then the numerator of the test statistic will become large and will translate into more evidence against the null hypothesis. On the other hand, if the group level averages are very similar to each other (Scenario 2) and therefore very similar to the overall mean, then the numerator of the test statistic will be smaller and there will be little evidence against the null hypothesis.
...
We haven’t said much about the denominators G-1 and \sum\limits_{g=1}^G(n_g-1) of the test statistic. Without going into too much detail, these denominators are used to provide averages. For example G-1 helps us estimate the average squared distance between the group means and the overall mean, while the second one, \sum\limits_{g=1}^G(n_g-1) helps us estimate the average squared distance within a group. These quantities are known as the degrees of freedom of the numerator and the degrees of freedom of the denominator, respectively.
